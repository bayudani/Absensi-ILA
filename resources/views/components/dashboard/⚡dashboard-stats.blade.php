    ...
    public function with(): array
    {
        ...
        elseif ($user->role === 'walas') {
            ...
            $stats = [
                ...
                'card3_val' => Absensi::whereIn('siswa_id', $siswaIds)
                    ->whereDate('tanggal', $today)
                    ->whereIn('status', ['I', 'S'])
                    ->count(),
...
<div wire:poll.15s>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        @if (!empty($stats))
            <!-- Card 1 -->
            <div
                class="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm hover:shadow-md transition-all duration-300">
            ...
            <div
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div
                        class="p-4 bg-{{ $stats['card3_color'] }}-50 text-{{ $stats['card3_color'] }}-600 rounded-2xl">
            ...
            <div
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div
                        class="p-4 bg-{{ $stats['card4_color'] }}-50 text-{{ $stats['card4_color'] }}-600 rounded-2xl">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden h-fit">
            <div class="p-6 border-b border-gray-50 bg-gray-50/50 flex justify-between items-center">
                <h3 class="text-xs font-black text-gray-700 uppercase tracking-[0.2em]">
                    <i class="fas fa-bolt mr-2 text-emerald-500"></i>Aktivitas
                    {{ $userRole === 'admin' ? 'Sekolah' : 'Kelas' }} Terbaru
                </h3>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @forelse($recentLogs as $log)
                            <tr class="hover:bg-emerald-50/20 transition-all">
                                <td class="p-6">
                                    <div class="font-bold text-gray-800 leading-none">{{ $log->siswa->nama_lengkap }}
                                    </div>
                                    <div class="text-[10px] text-gray-400 font-mono mt-1">NISN: {{ $log->siswa->nisn }}
                                    </div>
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="4"
                                    class="p-20 text-center text-gray-300 font-bold uppercase text-xs tracking-widest opacity-30">
                                    Belum ada aktivitas hari ini.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div class="space-y-6">
            <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100">
                <h4
                    class="font-black text-gray-700 text-[10px] uppercase tracking-widest mb-6 border-b pb-4 flex items-center">
                ...
                <div class="space-y-5">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <span class="text-sm text-emerald-600 font-black bg-emerald-50 px-3 py-1 rounded-xl"
                            id="live-clock">{{ now()->format('H:i:s') }}</span>
                    </div>
                    <div class="pt-4 border-t border-dashed">
                        <div class="p-4 bg-yellow-50 border border-yellow-100 rounded-2xl">
                            ...
                            <p class="text-xs text-yellow-700 font-bold leading-relaxed">Input absensi harian disarankan
                                selesai maksimal pukul 14:00 WIB.</p>
                        </div>
                    </div>
                </div>
            </div>
